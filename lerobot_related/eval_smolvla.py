...
class SmolVLAROSNode(Node):
    def __init__(self):
        ...
        self.sub_lang   = self.create_subscription(
            String, '/language_instruction', self.language_callback, 5)
    ...
    def language_callback(self, msg: String):
        if msg.data.strip():
            ...
            self.get_logger().info(f"Received new instruction: {self.current_instruction}")
    ...
    def control_loop(self):
        ...
        try:
            ...
            # 可选：限位、平滑（根据你的机器人添加）
            # action_np = np.clip(action_np, joint_min, joint_max)
